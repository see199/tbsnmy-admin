<?php
$event_list = array(
    array('第01場：',"2026-06-13 10:00AM"),
    array('第02場：',"2026-06-13 02:00PM"),
    array('第03場：',"2026-06-13 05:00PM"),
    array('第04場：',"2026-06-13 08:00PM"),
    array('第05場：',"2026-06-14 01:00AM"),
                            ...
                            <div class='col-xs-9 col-lg-6 text-left form-row'>
                                <div class='form form-group'>
                                    <select class='custom-combobox-input ui-widget ui-widget-content ui-state-default ui-corner-left' id='chapter_timezone' name='chapter_timezone'>
                                        <option value=''></option>
                                        <option value='Asia/Kuala_Lumpur'>馬來西亞 / 新加坡 / 汶萊</option>
                                        <option value='Asia/Taipei'>台灣</option>
                                        <option value='Asia/Hong_Kong'>香港</option>
                                        <option value='Asia/Bangkok'>泰國 / 越南 / 柬埔寨</option>
                                        <option value='Asia/Jakarta'>印尼（西部）</option>
                                        <option value='Asia/Makassar'>印尼（中部）</option>
                                        <option value='Asia/Jayapura'>印尼（東部）</option>
                                        <option value='Asia/Tokyo'>日本</option>
                                        <option value='Pacific/Guam'>關島</option>
                                        <option value='Australia/Sydney'>澳洲（東南部）</option>
                                        <option value='Indian/Christmas'>澳洲（聖誕島）</option>
                                        <option value='Australia/Perth'>澳洲（西部）</option>
                                        <option value='Pacific/Auckland'>紐西蘭</option>
                                        <option value='America/Sao_Paulo'>巴西（聖保羅）/（福塔雷薩）</option>
                                        <option value='America/Los_Angeles'>美國（西岸)/加拿大（BC省）</option>
                                        <option value='America/Denver'>美國（山地時區）/加拿大（AB省）</option>
                                        <option value='America/Chicago'>美國（中部時區）</option>
                                        <option value='America/New_York'>美國（東岸時區）/加拿大（ON/QC省）/巴拿馬</option>
                                        <option value='America/Puerto_Rico'>波多黎各/多米尼加共和國</option>
                                        <option value='Pacific/Honolulu'>夏威夷</option>
                                        <option value='Europe/London'>英國/葡萄牙</option>
                                        <option value='Europe/Berlin'>歐洲-1：瑞典/挪威/德國/瑞士/法國/西班牙/意大利</option>
                                        <option value='Europe/Helsinki'>歐洲-2：芬蘭/希臘</option>
                                        <option value='Africa/Johannesburg'>南非</option>
                                    </select>
                                </div>
                            </div>
                        </div>
    ...
    function updateLabel() {
        ...
        $.each(timezone_list, function(index, eventItem) {
            ...
            if (convertedDateTime.isValid) {
                ...
                $('#label_type_' + id).html(
                    eventDesc + "（當地時間：" + formattedTime + "）"
                );
            } else {
                // Keep the original text if conversion fails (e.g., invalid timezone ID)
                $('#label_type_' + id).html(eventDesc + "（時區轉換錯誤）");
            }
        });
    }

    // Removed: timezone_mapping, getTimeZoneList, and addHours functions.
